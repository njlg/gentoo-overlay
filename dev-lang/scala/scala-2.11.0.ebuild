...
src_compile() {
	...
	eant all.clean
	#eant -Djavac.args="-encoding UTF-8" -Djava6.home=${JAVA_HOME} -Dbuild.release=true build-opt
	eant -Djavac.args="-encoding UTF-8" -Dbuild.release=true build-opt
	...
	if use doc; then
		eant -Djavac.args="-encoding UTF-8" -Dbuild.release=true docs-opt
	fi

	eant -Djavac.args="-encoding UTF-8" -Dbuild.release=true fastdist-opt
}

src_test() {
	eant test || die "Some tests aren't passed"
}
